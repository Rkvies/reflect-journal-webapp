...
export const InsightsPanel: React.FC<InsightsPanelProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-2 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md shadow-indigo-600/20 flex-shrink-0 cursor-pointer"
          >
      ...
      {errorMessage && (
        <div className="p-3.5 rounded-2xl bg-rose-50/80 dark:bg-rose-950/60 backdrop-blur-md border border-rose-200/80 dark:border-rose-900/60 text-xs text-rose-700 dark:text-rose-300">
          {errorMessage}
        </div>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {latestInsight.themes && latestInsight.themes.map((th, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="p-5 rounded-3xl bg-white/75 dark:bg-slate-900/75 backdrop-blur-2xl border border-white/80 dark:border-white/10 flex flex-col justify-between space-y-3 shadow-sm hover:border-indigo-400 dark:hover:border-indigo-500/50 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg"
                  >
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm font-semibold font-serif text-slate-900 dark:text-white">{th.name}</span>
                        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full border backdrop-blur-xs ${
                          th.score >= 80 
