    ...
    >
      ...
      <div className="flex items-center justify-between gap-2.5 sm:gap-4">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-lg sm:text-2xl md:text-3xl font-serif font-bold text-slate-950 dark:text-white tracking-tight truncate">
              Hi, {firstName}
            </h2>
            <p className="text-[11px] sm:text-sm text-slate-700 dark:text-slate-200 font-medium line-clamp-1">
              {greetingSubtext}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          {saveStatus && (
            <span className="text-xs text-indigo-600 dark:text-indigo-300 flex items-center gap-1 font-medium animate-fade-in pr-1">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
        ...
        <div
          ...
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border backdrop-blur-md transition-all hover:-translate-y-0.5 whitespace-nowrap shadow-2xs ${
            streakCount > 0
              ? 'bg-amber-50/70 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border-amber-200/70 dark:border-amber-800/60'
              : 'bg-white/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border-white/60 dark:border-white/10'
          }`}
          title="Consecutive days with journal reflections"
        >
          <Flame
            className={`w-3.5 h-3.5 flex-shrink-0 ${
              streakCount > 0 ? 'text-amber-600 fill-amber-500' : 'text-slate-500'
            }`}
          />
        ...
        <div
          ...
          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-white/50 dark:bg-slate-800/50 backdrop-blur-md text-slate-700 dark:text-slate-300 border border-white/60 dark:border-white/10 whitespace-nowrap shadow-2xs hover:-translate-y-0.5 transition-all"
          title={
            latestEntryTitle
        ...
        {hasEntryToday && (
          <div
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50/70 dark:bg-emerald-950/40 backdrop-blur-md text-emerald-800 dark:text-emerald-300 border border-emerald-200/70 dark:border-emerald-800/60 whitespace-nowrap shadow-2xs hover:-translate-y-0.5 transition-all"
          >
